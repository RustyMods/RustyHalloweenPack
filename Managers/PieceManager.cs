...
public static class PiecePrefabManager
{
    ...
    private static void Patch_ObjectDBInit(ObjectDB __instance)
    {
        foreach (BuildPiece piece in BuildPiece.registeredPieces)
        {
            ...
            if (ZNetScene.instance && !piece.CloneDoorEffectsFrom.IsNullOrWhiteSpace() && piece.Prefab.TryGetComponent(out Door door))
            {
                var original = ZNetScene.instance.GetPrefab(piece.CloneDoorEffectsFrom);
