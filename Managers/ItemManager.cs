...
public class Item
{
    ...
    public static Dictionary<string, Item> m_itemByConfigGroup = new();
    ...
    internal static void Patch_ZNetSceneAwake(ZNetScene __instance)
    {
        foreach (Item item in registeredItems)
        {
            ...
            if (item.CloneEffectsFrom.IsNullOrWhiteSpace()) continue;
            var original = __instance.GetPrefab(item.CloneEffectsFrom);
            if (!original || !original.TryGetComponent(out ItemDrop component)) continue;
